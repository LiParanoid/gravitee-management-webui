...
class UserService {
  ...
  refreshEnvironmentPermissions(): ng.IPromise<User> {
    ...
    return this.EnvironmentService.getPermissions(this.Constants.org.currentEnv.id, this.Constants.org.currentEnv.id).then(response => {
  ...
  current(): ng.IPromise<User> {
    ...
    if (!this.currentUser || !this.currentUser.authenticated) {
      ...
      if (environmentId && environmentId[1]) {
        promises.push(this.EnvironmentService.getPermissions(environmentId[1], environmentId[1]));
      }
